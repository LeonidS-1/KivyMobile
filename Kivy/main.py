...
##############################################################################################################
    def texture_install(self):
        self.title = 'Trans-now-tor'
        self.icon = r'pictures\preview.png'
...
##############################################################################################################        
    def callback(self, instance, value):
        ...
        if value == 'normal' and instance.text == 'Архив':  
            ...
            self.archive() 
        if value == 'normal' and instance.text == 'Выход':   
            exit() 
...
##############################################################################################################    
    ...
    def archive(self):
        try: 
            self.remove(self.set_part_1, self.set_part_2)
            self.add(self.settings_btn)
        except: pass
        ...
        def change_pos(*change_crd):
            ...
            for num_block in range(len(self.block_list)-int(self.plus_change)): 
                ...
                self.block_list[num_block].text = f'{self.spisok[num_block+int(self.plus_change)][0]}\n{self.spisok[num_block][1]}'
